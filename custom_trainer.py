...
class CustomTrainer(Trainer):
    def __init__(self, extra_losses: List[str] = None, **kwargs):
        # 只处理 extra_losses，其他所有 Trainer 参数通过 kwargs 传递
        self.extra_losses = extra_losses or []
        super().__init__(**kwargs)

        if self.extra_losses:
            self.add_callback(AddExtraLossesToTrainerState(self.extra_losses))
    ...
    def _maybe_log_save_evaluate(self, tr_loss, grad_norm, model, trial, epoch):
        """支持 extra_losses，同时兼容 transformers >=4.30 内部调用"""
        if self.control.should_log and self.state.global_step > self._globalstep_last_logged:
            ...
            logs.update(unwrap_model(model).get_extra_logging_dict() if hasattr(unwrap_model(model), "get_extra_logging_dict") else {})
        ...
        if self.control.should_evaluate:
            ignore_keys = getattr(self.args, "ignore_keys_for_eval", None)
            metrics = self.evaluate(ignore_keys=ignore_keys)
